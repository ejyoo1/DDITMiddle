	...
	// Slideshow Background.
		(function() {
			...
			// Main loop.
				...
				// Bail if we only have a single BG or the client doesn't
				// support transitions.
					if ($bgs.length == 1
					||	!canUse('transition'))
						return;
		...
		(function() {
			...
			// Note: If you're *not* using AJAX, get rid of this event listener.
				$form.addEventListener('submit', function(event) {
					...
					// Process form.
					// Note: Doesn't actually do anything yet (other than report
					// back with a "thank you"),
					// but there's enough here to piece together a working AJAX
					// submission call that does.
						window.setTimeout(function() {
							...
							// Show message.
								$message._show('success', '가입해 주셔서 감사합니다!');
								// $message._show('failure', 'Something went
